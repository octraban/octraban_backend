    ...
    const searchIndexEntries = await (prismaRead as any).searchIndexEntry.findMany({
      where: {
        AND: [
          ...
          functionMatch ? { AND: [{ contentType: 'function' }, { content: { contains: functionMatch, mode: 'insensitive' } }] } : undefined,
          importMatch ? { AND: [{ contentType: 'import' }, { content: { contains: importMatch, mode: 'insensitive' } }] } : undefined,
          eventMatch ? { AND: [{ contentType: 'event' }, { content: { contains: eventMatch, mode: 'insensitive' } }] } : undefined,
          storageMatch ? { AND: [{ contentType: 'storage' }, { content: { contains: storageMatch, mode: 'insensitive' } }] } : undefined,
          errorMatch ? { AND: [{ contentType: 'error' }, { content: { contains: errorMatch, mode: 'insensitive' } }] } : undefined,
        ].filter(Boolean),
      },
...
searchRouter.get('/index', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ indexed, message: `Reindexed ${indexed} entries from ${sources.length} contracts` });
  } catch (err: any) {
